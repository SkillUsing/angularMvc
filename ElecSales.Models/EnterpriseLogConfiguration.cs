...
{
    // EnterpriseLog
    public partial class EnterpriseLogConfiguration : EntityTypeConfiguration<EnterpriseLog>
    {
        public EnterpriseLogConfiguration()
            : this("dbo")
        {
        }
 
        public EnterpriseLogConfiguration(string schema)
        {
            ToTable(schema + ".EnterpriseLog");
            HasKey(x => x.Id);

            Property(x => x.Id).HasColumnName("Id").IsRequired().HasColumnType("int").HasDatabaseGeneratedOption(DatabaseGeneratedOption.Identity);
            Property(x => x.Type).HasColumnName("Type").IsRequired().HasColumnType("int");
            Property(x => x.LogContent).HasColumnName("LogContent").IsRequired().HasColumnType("nvarchar").HasMaxLength(2000);
